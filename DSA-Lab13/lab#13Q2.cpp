...
void bubbleSort(Record records[], int size) {
    for (int i = 0; i < size - 1; ++i) {
        for (int j = 0; j < size - i - 1; ++j) {
            if (records[j].sapID < records[j + 1].sapID) { // Changed to <
...
void insertionSort(Record records[], int size) {
    for (int i = 1; i < size; ++i) {
        ...
        while (j >= 0 && records[j].sapID < key.sapID) { // Changed to <
...
int main() {
    ...
    cout << "Records after Bubble Sort (Descending Order):\n";
    ...
    cout << "Records after Insertion Sort (Descending Order):\n";
